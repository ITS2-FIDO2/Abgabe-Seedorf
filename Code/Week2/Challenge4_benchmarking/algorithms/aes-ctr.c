...
void executeAesCtr(int numberOfRounds, int keySize, size_t messageLength){
    for(int round = 1; round <= numberOfRounds; round++){
        ...
        uint8_t* counter_block = malloc(AES_BLOCK_SIZE);
        random_bytes(counter_block, AES_BLOCK_SIZE);
        cipher_t cipher;
        ...
        cipher_encrypt_ctr(&cipher, counter_block, 8, input, total, output);
        ...
        cipher_decrypt_ctr(&cipher, counter_block, 8, output, total, decryptBuffer);
